...
| 3 | **Chat with your brain** | Grounded Q&A with citations over the index; master-context doc (mini Master Prompt) | **built 2026-07-30**: `/ask` (one retrieval, grounded, cited) and `/chat` (the model runs its own searches, up to 4 rounds, with the trace shown). Current-work context pulled forward earlier the same day. **Reframed:** the "master-context doc" should be a derived *corpus map* — the owner's own vocabulary per era, so an agent can translate a vague ask into his words before searching. See `ai/PROJECT_STATE.md` |
| 4 | **Review + resurfacing** | AI weekly review digest (themes, resurfaced items vs. Favorite Problems, queue hygiene), review mode | planned — **thematic synthesis is the same machinery and now has evidence**: a 2026-07-30 prototype over all 4,259 dated docs produced a readable intellectual timeline (2016 deep-work/philosophy → 2018 music marketing → 2021 algorithms/beats → 2024 hiring/rails). Statistics propose the themes, the model narrates them with citations. Detail in `ai/PROJECT_STATE.md` |
